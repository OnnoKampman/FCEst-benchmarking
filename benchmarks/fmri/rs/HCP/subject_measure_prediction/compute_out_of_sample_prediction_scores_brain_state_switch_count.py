...
if __name__ == '__main__':
    ...
    metric = 'correlation'
    n_basis_states = 3
    subject_features_type = 'brain_state_switch_count'
    ...
    tvfc_estimation_method = sys.argv[3]  # 'SVWP_joint', 'DCC_joint', 'DCC_bivariate_loop', 'SW_cross_validated', 'SW_30', or 'SW_60'
    ...
    # We use the brain state switch counts as predictive features, aiming to predict subject measures.
    ...
    n_brain_state_switches_filepath = os.path.join(
        cfg['git-results-basedir'], 'brain_states', f'k{n_basis_states:d}',
        f'number_of_brain_state_switches_{tvfc_estimation_method:s}.csv'
    )  # (n_subjects, n_scans)
    n_brain_state_switches_df = pd.read_csv(
        n_brain_state_switches_filepath,
        index_col=0
    )  # (n_subjects, n_scans)
    print(n_brain_state_switches_df.shape)

    #set x data to be the input variable you want to use
    X = n_brain_state_switches_df.values  # (n_subjects, n_features)
    ...
    linear_ridge_model_results_r2_df.to_csv(
        os.path.join(
            scores_savedir, f'{metric:s}_linear_ridge_model_r2_scores_{subject_features_type:s}_{tvfc_estimation_method:s}.csv'
        ),
        float_format="%.7f"
    )
    linear_ridge_model_results_explained_variance_df.to_csv(
        os.path.join(
            scores_savedir, f'{metric:s}_linear_ridge_model_variance_explained_{subject_features_type:s}_{tvfc_estimation_method:s}.csv'
        ),
        float_format="%.7f"
    )
    linear_ridge_model_results_prediction_accuracy_df.to_csv(
        os.path.join(
            scores_savedir, f'{metric:s}_linear_ridge_model_prediction_accuracy_{subject_features_type:s}_{tvfc_estimation_method:s}.csv'
        ),
        float_format="%.7f"
    )
