...
if __name__ == '__main__':
    ...
    all_subjects_list = get_human_connectome_project_subjects(
        data_dir=cfg['data-dir'], first_n_subjects=cfg['n-subjects'], as_ints=True
    )
    ...
    for i_tvfc_summary_measure, tvfc_summary_measure in enumerate(cfg['TVFC-summary-measures']):
        for i_subject_measure, subject_measure in enumerate(subject_measures_list):
            for morphometricity_model in cfg['morphometricity-models']:
                ...
                logging.info(f"> TVFC summary measure {i_tvfc_summary_measure+1:d}/{n_tvfc_summary_measures:d}: '{tvfc_summary_measure:s}'")
                logging.info(f"> Subject measure {i_subject_measure+1:02d}/{n_subject_measures:02d}:    '{subject_measure:s}'")
                logging.info(f"> TVFC estimation model:    '{morphometricity_model:s}'")
                ...
                morphometricity_results_df = pd.DataFrame(
                    np.nan,
                    index=np.arange(n_time_series),
                    columns=np.arange(n_time_series)
                )

                for (edge_tuple_i, edge_tuple_j) in get_all_lower_triangular_indices_tuples(n_time_series):
                    logging.info(f"Edge ({edge_tuple_i:d}, {edge_tuple_j:d})...")
                    ...
                    morphometricity_results_df.loc[edge_tuple_i, edge_tuple_j] = morphometricity_score
                ...
                scores_savedir = os.path.join(
                    ...
                    tvfc_summary_measure, subject_measure, morphometricity_model
                )
