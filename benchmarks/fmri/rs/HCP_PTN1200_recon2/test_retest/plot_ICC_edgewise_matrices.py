...
def _plot_icc_scores_per_edge(
        config_dict: dict, icc_edgewise_df: pd.DataFrame, tvfc_summary_measure: str,
        model_name: str, metric: str, figures_savedir: str
) -> None:
    """
    ...
    if figures_savedir is not None:
        figure_name = f'{metric:s}_{tvfc_summary_measure:s}_ICCs_{model_name:s}.pdf'
        plt.savefig(
...
def _plot_icc_scores_per_edge_two_model_scatterplot(
        summary_measure_icc_df_1: pd.DataFrame, summary_measure_icc_df_2: pd.DataFrame,
        model_name_1: str, model_name_2: str,
        tvfc_summary_measure: str, metric: str,
        figures_savedir: str,
        ...
        for model_name in cfg['test-retest-models']:
            ...
            if os.path.exists(icc_scores_filepath):
                summary_measure_icc_df = pd.read_csv(
                    icc_scores_filepath, index_col=0
                )
                _plot_icc_scores_per_edge(
                    config_dict=cfg,
