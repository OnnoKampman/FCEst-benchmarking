...
def _plot_quantitative_results(
        config_dict: dict,
        performance_metric: str, noise_type: str, data_split: str,
        bbox_to_anchor=(1.01, 1.0), markersize: float = 3.0, capsize: float = 2.5,
        stddev_error_bars: bool = True, figures_savedir: str = None
) -> None:
    """
    ...
    average_quantitative_results_df = pd.read_csv(
        os.path.join(
            config_dict['git-results-basedir'], noise_type, data_split, f'{performance_metric:s}_mean.csv'
        ),
        index_col=0
    )
    if stddev_error_bars:
        stddev_quantitative_results_df = pd.read_csv(
            os.path.join(
                config_dict['git-results-basedir'], noise_type, data_split, f'{performance_metric:s}_std.csv'
            ),
            index_col=0
        )
    else:
        stderr_quantitative_results_df = pd.read_csv(
            os.path.join(
                config_dict['git-results-basedir'], noise_type, data_split, f'{performance_metric:s}_se.csv'
            ),
            index_col=0
        )

    average_quantitative_results_df = _process_df(config_dict, average_quantitative_results_df)
    if stddev_error_bars:
        errorbars_quantitative_results_df = _process_df(config_dict, stddev_quantitative_results_df)
    else:
        errorbars_quantitative_results_df = _process_df(config_dict, stderr_quantitative_results_df)
    ...
    # palette_tab10 = sns.color_palette("tab10", 10)  # for Fig. S1 only
    # cmap = matplotlib.colormaps['Set1']  # for Fig. S1 only
    ...
    for i_model, model in enumerate(models):
        ...
        plt.errorbar(
            ...
            color=cmap(i_model + 1) if config_dict['data-set-name'] == 'd2' and i_model > 1 else cmap(i_model),  # this line makes sure colors are assigned to the same model across plots
            # color=cmap(i_model),  # for Fig. S1 only
            # color=cmap[i_model],  # for Fig. S1 only
...
def _process_df(
        config_dict: dict, quant_results_df: pd.DataFrame
) -> pd.DataFrame:

    quant_results_df = _select_models_and_covariance_structures_to_plot(config_dict, quant_results_df)
    ...
    quant_results_df.columns = quant_results_df.columns.str.replace('checkerboard', 'boxcar')
...
def _select_models_and_covariance_structures_to_plot(
        config_dict: dict, df: pd.DataFrame
) -> pd.DataFrame:
    """
    ...
    # models_to_plot = [  # for Fig. S1 only
    #     'SW_cross_validated',
    #     'SW_15',
    #     'SW_30',
    #     'SW_60',
