        ...
        for covs_type in cfg['all-covs-types']:
            slurm_job_str = ("#!/bin/bash\n"
                            ...
                            #  "conda activate fcest-env\n"
                             ...
                             f"srun --cpu_bind=threads --distribution=block:block python ./benchmarks/simulations/train_models/train_WP.py {data_set_name:s} {experiment_data:s} {model_name:s} {data_split:s} {noise_type:s} {covs_type:s}")
            print('\n', slurm_job_str, '\n')
