...
"""
TODO: set up for SLURM to paralellize downloading

This script downloads data from the NKI Rockland Public releases stored in the cloud in BIDS format.
You can specify sex, age range, handedness, session, scan type (anatomical, functional, dwi) and to limit your download to a subset of the sample.
...
# http://fcon_1000.projects.nitrc.org/indi/enhanced/study_codes.html
SESSIONS = [
    'BAS1',
    # 'BAS2',
    # 'FLU1',
...
def generate_Subfolders(s3_client):
    ...
    gen_subfolders = files(
        s3_client, 'fcp-indi', prefix=('data/Projects/RocklandSample/RawDataBIDSLatest/')
    )#+extra
...
def collect_and_download(
        out_dir: str,
        aws_links='aws_links.csv', less_than=0, greater_than=0,
        sex='', handedness='', sessions=SESSIONS, scans=SCANS,
        series=SERIES_MAP.keys(), derivatives=False, dryrun=False
) -> None:
    """
    Function to collect and download images from the Rockland sample directory on FCP-INDI's S3 bucket.
    Parameters
    ----------
    out_dir : string
    ...
    if not os.path.exists(out_dir) and not dryrun:
        print(f"Could not find '{out_dir:s}', creating now...")
        os.makedirs(out_dir)

    if os.path.exists(aws_links):
        participants_df = pandas.read_csv(aws_links, na_values=['n/a'])
    else:
        # Note: make sure `wget` is installed on your machine.
    ...
    if len(participants_df) == 0:
        ...
        print('Participants DataFrame:', participants_df.shape)
        print('Number of unique subjects:', len(participants_df['subject'].unique()))
    ...
    if not dryrun:
        # Save out revised participants.tsv to output directory.
        # If a participants.tsv already exists, open it and append it to the new one.
